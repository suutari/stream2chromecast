"""
Translates messages sent to & received from a Chromecast
...
Thanks to TheCrazyT for this very helpful gist:
...
def format_string_field(field_number, field_data):
    ...
    field = pack("B",
                 # 2 = Length-delimited field type
                 format_field_id(field_number, 2))
...
def extract_length_header(msg):
    """ extracts the length header from the first 4 bytes of
    a received message """
...
def extract_string_field(data):
    """ extracts a protocol buffers length-delimited field from
    a received message """
