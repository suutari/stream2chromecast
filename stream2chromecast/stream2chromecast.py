...
FFMPEG = (
    'ffmpeg -i "%s" -preset ultrafast -f mp4'
    ' -frag_duration 3000 -b:v 2000k -loglevel error %s -')
AVCONV = (
    'avconv -i "%s" -preset ultrafast -f mp4'
    ' -frag_duration 3000 -b:v 2000k -loglevel error %s -')
...
class TranscodingRequestHandler(RequestHandler):
    ...
    which require realtime transcoding with ffmpeg """
    ...
    def write_response(self, filepath):
        ...
        ffmpeg_command = self.transcoder_command % (
            filepath, self.transcode_options)
...
def get_transcoder_cmds(preferred_transcoder=None):
    """ establish which transcoder utility to use
    ...
    # if anything other than avconv is preferred, try to use ffmpeg otherwise
    # use avconv
...
def kill_old_pid(device_ip):
    ...
    casting to the specified device. """
...
def save_pid(device_ip):
    """ saves the process id of this application
    casting to the specified device in a pid file. """
    pid_file = PIDFILE % device_ip
...
def get_mimetype(filename, ffprobe_cmd=None):
    ...
    if guess is not None:
        if guess.lower().startswith("video/") or (
                guess.lower().startswith("audio/")):
            mimetype = guess

    # use the OS file command...
    try:
        file_cmd = 'file --mime-type -b "%s"' % filename
        file_mimetype = subprocess.check_output(
            file_cmd, shell=True).strip().lower()

        if file_mimetype.startswith("video/") or (
                file_mimetype.startswith("audio/")):
    ...
    ffprobe_cmd = '%s -show_streams -show_format "%s"' % (
        ffprobe_cmd, filename)
    ffmpeg_process = subprocess.Popen(
        ffprobe_cmd, stdout=subprocess.PIPE, shell=True)
...
def play(filename, transcode=False, transcoder=None, transcode_options=None,
    ...
    url = "http://%s:%s/%s" % (
        webserver_ip, str(server.server_port),
        urllib.quote_plus(filename, "/"))

    print "URL & content-type: ", url, req_handler.content_type

    # create another webserver to handle a request for the subtitles file, if
    # specified in the subtitles parameter
    sub = None
    ...
    if subtitles:
        if os.path.isfile(subtitles):
            ...
            sub_server = BaseHTTPServer.HTTPServer(
                (webserver_ip, sub_port), SubRequestHandler)
            thread2 = Thread(target=sub_server.handle_request)
            thread2.start()

            sub = "http://%s:%s%s" % (
                webserver_ip, str(sub_server.server_port),
...
def list_devices():
    ...
    device_ips = cc_device_finder.search_network(
        device_limit=None, time_limit=10)
...
def parse_args():
    ...
    device_group.add_argument(
        "-d", "--device_name",
        help=("specify an Chromecast device by name (or ip address)"
              " explicitly: e.g. to play a file on a specific device"),
        default=None)
    ...
    transcoder_group.add_argument(
        "--transcoder", choices=["ffmpeg", "avconv"], default="ffmpeg")
    transcoder_group.add_argument(
        "--transcode_options",
        ...
        default=None)
    transcoder_group.add_argument(
        "--transcode_bufsize", type=int,
        help=("pecify the buffer size of the data returned from the"
              " transcoder. Increasing this can help when on a slow network."),
        default=0)

    parser = argparse.ArgumentParser()
    subparsers = parser.add_subparsers()

    devices_list_parser = subparsers.add_parser(
        "devices_list",
        help="Search for all Chromecast devices on the network")
    devices_list_parser.set_defaults(function=list_devices)

    play_parser = subparsers.add_parser(
        "play", parents=[device_parser, server_parser, subtitles_parser,
                         transcoder_parser],
        help="Play a file")
    play_parser.add_argument("filename", help="The file to play")
    play_parser.set_defaults(function=play)

    play_url = subparsers.add_parser(
        "playurl", parents=[device_parser],
    ...
    continue_parser = subparsers.add_parser(
        "continue", parents=[device_parser],
    ...
    setvol_parser = subparsers.add_parser("setvol", parents=[device_parser],
                                          help="Set the volume")
    setvol_parser.add_argument(
        "volume", type=float,
        help="value between 0 & 1.0  (e.g. 0.5 = half volume)")
    ...
    volume_down_parser = subparsers.add_parser(
        "voldown", parents=[device_parser],
